...
const fetch_book_logicparam = (req, res) => {
...
const add_book_logic = (req, res) => {
  ...
  if (Object.keys(bookdata).length == 6) {
    authbook
      .exists({ bookname: req.body.bookname }) // here is a first bookneme is schema bookname and second bookname is reqbody book nam
      .then((data) => {
        if (data) {
